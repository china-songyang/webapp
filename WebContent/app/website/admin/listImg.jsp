...
<script type="text/javascript">
		$(function() {
			...
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/website/admin/queryImg.action?id=<%=request.getParameter("id")%>',
				idField : 'id',//唯一标识列
				sortName : 'id',
				sortOrder : 'desc',
				remoteSort : true,
				...
				toolbar : [ {
					...
					iconCls : 'icon-add',
					handler : function() {
						window.location.href='<%=request.getContextPath()%>/app/website/admin/addImg.jsp?id=<%=request.getParameter("id")%>';
					...
					handler : function() {
						...
						$.messager.confirm('确认删除项', '确认删除该选项', function(result){
							if (result){
								window.location.href='<%=request.getContextPath()%>/app/website/admin/delete.action?ids='+ids.join('__');
							}
						});
						return false;
					}
		...
		function editVO(id){
			window.location.href='<%=request.getContextPath()%>/app/website/admin/queryById.action?id='+ id;
			return false;
		}
	</script>
</head>
			...
			<table>
				<tr align="right">
					...
					<td><input id="createTime" name="createTime"
						class="easyui-datebox" type="text"></input></td>
					<td>是否显示:</td>
					<td><select id="state" class="easyui-combobox" name="state"
						...
						valueField="id" textField="text" mode="remote"></select></td>
				</tr>
			</table>
