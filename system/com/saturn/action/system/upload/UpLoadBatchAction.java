...
public class UpLoadBatchAction implements IAction {

	public String requestMapping() {
		return "/app/system/upload/batchupload.do";
	}
			...
			while (itr.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					if (!Arrays.<String> asList(extMap.get(dirName).split(","))
							.contains(fileExt)) {
						return new JsonView(getInfo("上传文件扩展名是不允许的扩展名。只允许"
					...
					return new JsonView("{\"error\":0, \"url\":\"" + saveUrl
							+ newFileName + "\"}");
				}
			}
